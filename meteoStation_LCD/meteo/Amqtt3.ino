...
void do_send_mqtt_pin_out_color(uint8_t pin, uint8_t val)
{
  ...
  {
    ret = client_mqtt.publish(mqtt_out1_topic_send_color.c_str(), 0, mqtt_retain, String(val).c_str() );
...
void do_send_mqtt_pin_out(uint8_t pin, uint8_t val)
{
  ...
  {
    ret = client_mqtt.publish(mqtt_out1_topic_send.c_str(), 0, mqtt_retain, String(val).c_str() );
#ifdef debug
  ...
  {
    ret = client_mqtt.publish(mqtt_out2_topic_send.c_str(), 0, mqtt_retain, String(val).c_str() );
#ifdef debug
  ...
  {
    ret = client_mqtt.publish(mqtt_out3_topic_send.c_str(), 0, mqtt_retain, String(val).c_str() );
#ifdef debug
  ...
  {
    ret = client_mqtt.publish(mqtt_out4_topic_send.c_str(), 0, mqtt_retain, String(val).c_str() );
#ifdef debug
  ...
  {
    ret = client_mqtt.publish(mqtt_out5_topic_send.c_str(), 0, mqtt_retain, String(val).c_str() );
#ifdef debug
...
void do_send_song_mqtt(char ch)
{
...
#ifdef debug
  Serial.println(F("---MQTT3_S"));
#endif

  if (ch == 's' && mqtt_song_topic_send.length() > 1)
  {
    ret = client_mqtt.publish(mqtt_song_topic_send.c_str(), 0, mqtt_retain, play_song.c_str() );
#ifdef debug
  ...
  {
    ret = client_mqtt.publish(mqtt_plist_topic_send.c_str(), 0, mqtt_retain, plist_send.c_str() );
#ifdef debug
...
void do_send_mqtt()
{
  ...
  // in mqtt_1  if (play_bg) return; //дальше тормоза
  //TTTTTTTTTTTTTTTTTTTT
  if (mqtt_T1_topic_send != "" && temp_kv != -100)
  {
    ret = client_mqtt.publish(mqtt_T1_topic_send.c_str(), 0, mqtt_retain, String(temp_kv).c_str() );
#ifdef debug
  ...
  {
    ret = client_mqtt.publish(mqtt_H_topic_send.c_str(), 0, mqtt_retain, String(h_kv).c_str() );
  ...
  {
    ret = client_mqtt.publish(mqtt_T0_topic_send.c_str(), 0, mqtt_retain, String(temp_u).c_str() );
  ...
  {
    ret = client_mqtt.publish(mqtt_P_topic_send.c_str(), 0, mqtt_retain, String(P).c_str() );
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total)
{
  ...
  if (mqtt_CMD_topic_read == String(topic))
  {
    ...
    {
      async_str_mqtt[0] = '=';
      do_parse_str(async_str_mqtt) ;
      return;
    }
    else
      return; //cmd async_str_mqtt loop
  }
  //CMD
  ...
  if (mqtt_out1_topic_read_color == String(topic) )
  {
    ...
    async_str_mqtt += buf;
#ifdef debug
    Serial.print(F("out1 color_R:")); Serial.print(async_str_mqtt);
#endif
    do_parse_str(async_str_mqtt) ;
    return;
  }
  else if (mqtt_out1_topic_read == String(topic) ) //яркость
  {
    ...
    do_parse_str(async_str_mqtt) ;
#ifdef debug
    Serial.print(F("out1 read:")); Serial.print(async_str_mqtt);
#endif
    return;
  }
#else //color end
...
#ifdef vcc_read
  //VCC
  if (mqtt_VCC_topic_read == String(topic))
  {
    vcc = pay_int;
#ifdef debug
...
void onMqttConnect(bool sessionPresent) {
  Serial.println("onMqttConnect MQTT3");
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_out1_topic_read.c_str(), 0);
#ifdef debug
    Serial.print(mqtt_out1_topic_read + F(" subscribe:"));
    Serial.print(F(" ret:"));   Serial.println(ret);
#endif
  }
#ifdef pin_out1_color_leds
  if (mqtt_out1_topic_read_color != "")
  {
    ret =  client_mqtt.subscribe(mqtt_out1_topic_read_color.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_out1_topic_send_color.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_out2_topic_read.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_out3_topic_read.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_out4_topic_read.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_out5_topic_read.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_CMD_topic_read.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_H_topic_read.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_T1_topic_read.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_T0_topic_read.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_P_topic_read.c_str(), 0);
#ifdef debug
  ...
  {
    ret =  client_mqtt.subscribe(mqtt_VCC_topic_read.c_str(), 0);
#ifdef debug
...
#ifdef debug
  Serial.println(F("onMqttDisconn"));
#endif
  if (WiFi.isConnected()) {
...
/*
  ...
  #ifdef debug
  Serial.println(F("onMqttSubscribe"));
  #endif
  }
